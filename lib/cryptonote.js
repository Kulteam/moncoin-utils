...
const Crypto = require('./moncoin-crypto')
const Mnemonic = require('./mnemonic')
const Numeral = require('numeral')
const SecureRandomString = require('secure-random-string')
const Transaction = require('./transaction')
const MONCoinCrypto = new Crypto()
...
class CryptoNote {
   ...
   * @property {number} [addressPrefix=2199681] - The decimal representation of the wallet address prefix
   ...
   * @param {number} [addressPrefix=2199681] - decimal representation of the CryptoNote address prefix
  ...
  underivePublicKey (derivation, outputIndex, outputKey) {
    ...
    const [err, key] = MONCoinCrypto.underivePublicKey(derivation, outputIndex, outputKey)
...
function cnFastHash (input) {
  ...
  const [err, hash] = MONCoinCrypto.cn_fast_hash(input)
...
function derivePublicKey (derivation, outputIndex, publicKey) {
  ...
  const [err, key] = MONCoinCrypto.derivePublicKey(derivation, outputIndex, publicKey)
...
function deriveSecretKey (derivation, outputIndex, privateKey) {
  ...
  const [err, key] = MONCoinCrypto.deriveSecretKey(derivation, outputIndex, privateKey)
...
function generateDeterministicSubwalletKeys (privateKey, walletIndex) {
  ...
  const [err, keys] = MONCoinCrypto.generateDeterministicSubwalletKeys(privateKey, walletIndex)
...
function generateKeyDerivation (transactionPublicKey, privateViewKey) {
  ...
  const [err, derivation] = MONCoinCrypto.generateKeyDerivation(transactionPublicKey, privateViewKey)
...
function generateKeyImage (publicKey, privateKey) {
  ...
  const [err, keyImage] = MONCoinCrypto.generateKeyImage(publicKey, privateKey)
...
function checkRingSignatures (transactionPrefixHash, keyImage, inputKeys, signatures) {
  ...
  if (userCryptoFunctions.checkRingSignatures) {
    ...
    return MONCoinCrypto.checkRingSignatures(transactionPrefixHash, keyImage, inputKeys, signatures)
  }
}
...
function generateRingSignaturesPrimitive (transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex) {
  ...
  if (userCryptoFunctions.generateRingSignatures) {
    ...
    const [err, sigs] = MONCoinCrypto.generateRingSignatures(transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex)
...
function generateSignature (digestHash, publicKey, privateKey) {
  ...
  if (userCryptoFunctions.generateSignature) {
    ...
    [err, signature] = MONCoinCrypto.generateSignature(digestHash, publicKey, privateKey)
  }
...
function privateKeyToPublicKey (privateKey) {
  ...
  const [err, key] = MONCoinCrypto.secretKeyToPublicKey(privateKey)
...
function scReduce32 (hex) {
  const [err, result] = MONCoinCrypto.scReduce32(hex)
...
function verifySignature (digestHash, publicKey, signature) {
  ...
  return MONCoinCrypto.checkSignature(digestHash, publicKey, signature)
}
