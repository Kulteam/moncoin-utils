...
class Transaction {
  ...
  get hash () {
    ...
    const [err, hash] = MONCoinCrypto.cn_fast_hash(this.cached.blob)

    /* Version 2 transactions are actualy double hashed to get the hash */
    if (this.version >= 2) {
      const [err, hash2] = MONCoinCrypto.cn_fast_hash(hash + TransactionVersion2Suffix)
  ...
  get prefixHash () {
    ...
    const [err, hash] = MONCoinCrypto.cn_fast_hash(this.cached.prefix)
