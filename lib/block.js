...
class Block {
  ...
  _fromBlob (blob) {
    ...
    if (this.majorVersion >= this.activateParentBlockVersion) {
      ...
      const [btbderr, baseTransactionBranchDepth] = MONCoinCrypto.tree_depth(this.parentBlock.transactionCount)
...
function calculateBaseTransactionBranch (block) {
  ...
  const [err, treeBranch] = MONCoinCrypto.tree_branch(transactions)
...
function calculateTransactionTreeHashData (block) {
  const transactions = [block.minerTransaction.hash]
  block.transactions.forEach(txn => transactions.push(txn))
  const [err, treeHash] = MONCoinCrypto.tree_hash(transactions)
...
function getBlockHash (blob) {
  ...
  const [err, hash] = MONCoinCrypto.cn_fast_hash(writer.blob)
...
function getBlockPoWHash (majorVersion, blob) {
  ...
  switch (majorVersion) {
    ...
    case 2:
    case 3:
      [err, hash] = MONCoinCrypto.cn_slow_hash_v0(blob)
      break
    case 4:
      [err, hash] = MONCoinCrypto.cn_lite_slow_hash_v1(blob)
      break
    case 5:
      [err, hash] = MONCoinCrypto.cn_turtle_lite_slow_hash_v2(blob)
      break
    case 6:
      [err, hash] = MONCoinCrypto.chukwa_slow_hash(blob)
      break
    default:
...
function toHashingBlob (block, headerOnly) {
  ...
  if (block.majorVersion >= block.activateParentBlockVersion) {
    ...
    const [parentErr, parentTreeHash] = MONCoinCrypto.tree_hash_from_branch(block.parentBlock.baseTransactionBranch, block.parentBlock.minerTransaction.hash, 0)
